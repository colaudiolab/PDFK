...
class ERLearner(BaseLearner):
    def __init__(self, args):
        ...
        self.previous_model = None  # for feature drift computation only
    ...
    def train(self, dataloader, **kwargs):
        ...
        eval_hook = kwargs.get("eval_hook", None)  # ==== 新增：取出钩子 ====
        ...
        for j, batch in enumerate(dataloader):
            ...
            for _ in range(self.params.mem_iters):
                ...
                if mem_x.size(0) > 0:
                    # Combined batch
                    combined_x, combined_y = self.combine(batch_x, batch_y, mem_x,
                                                          mem_y)  # (batch_size, nb_channel, img_size, img_size)
                    ...
                    if eval_hook is not None:
                        eval_hook.maybe(self.model, self.global_step)
            ...
            if (j == (len(dataloader) - 1)) and (j > 0):
                if self.params.tsne and task_id == self.params.n_tasks - 1:
                    print(
                        f"[DEBUG] tsne conditions: j={j}, len={len(dataloader)}, tsne_flag={self.params.tsne}, task_id={task_id}")
                    self.tsne()
                print(

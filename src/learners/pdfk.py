...
from src.models.resnet import ResNet18, WTResNet18, ImageNet_ResNet18
...
from src.utils.perturber import Perturber
...
class PDFKLearner(ERLearner):
    def __init__(self, args):
        ...
        if self.params.alpha_min is None or self.params.alpha_max is None:
            ...
            for i in range(self.params.n_teacher):
                ...
                self.ema_alphas[i] = 10 ** (
                        ...
                        (np.log10(self.params.alpha_max) - np.log10(self.params.alpha_min)) * i / (
                            max(1, self.params.n_teacher - 1))
                )
        ...
        self.perturber = Perturber(LearnerWrapper(self.model, device, self.params))
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=True):
                ...
                for _ in range(self.params.mem_iters):
                    ...
                    if mem_x.size(0) > 0:
                        ...
                        self.optim.zero_grad()  # 必须在 STAR 和主任务之前执行
                        # 扰动梯度
                        if not self.buffer.is_empty():
                            with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=False):  # STAR 不支持 AMP
                                buf_x_raw, buf_y_raw = self.buffer.random_retrieve(n_imgs=self.params.mem_batch_size)
                                buf_x_raw, buf_y_raw = buf_x_raw.to(self.device), buf_y_raw.to(self.device)

                                # buf_x, buf_y = get_correctly_classified(self.model, buf_x_raw, buf_y_raw)
                                if buf_x_raw.size(0) > 0:  # 必须检查是否筛完为空
                                    self.perturber(buf_x_raw, buf_y_raw)
                        ...
                        for teacher in self.ema_models.values():
                            ...
                            if self.params.no_aug:
                                ...
                                loss_dist += (self.wkdloss(logits_tea.detach(), logits_stu) + self.wkdloss(
                                    logits_tea.detach(), logits_stu_raw)) / 2
                        ...
                        print(f"Phase: {task_name}  Loss:{loss.item():.3f}  Loss dist:{loss_dist.item():.3f} batch {j}",
                              end="\r")
    ...
    def update_ema(self, init=False):
        ...
        for i, ema_model in enumerate(self.ema_models.values()):
            ...
            for param, ema_param in zip(self.model.parameters(), ema_model.parameters()):
                ...
                if init:
                    ema_param.data.mul_(0).add_(p * alpha / (1 - alpha ** max(1, self.stream_idx // (
                                self.params.batch_size * self.params.ema_correction_step))))
                else:
                    ema_param.data.mul_(1 - alpha).add_(p * alpha / (1 - alpha ** max(1, self.stream_idx // (
                                self.params.batch_size * self.params.ema_correction_step))))
...
def get_correctly_classified(model, x, y):
    with torch.no_grad():

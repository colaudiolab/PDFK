...
class MultiBranchResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf, bias=True, dim_in=512, instance_norm=False, input_channels=3, num_branches=3, ret_feat=True):
        """
        ResNet-18 多分支结构 (layer3 共享，layer4 分支)

        参数:
        - block: 残差块 (BasicBlock)
        - num_classes: 分类类别数
        - nf: 初始通道数 (ResNet-18 采用 64)
        - bias: 线性层是否使用 bias
        ...
        self.in_planes = nf  # ResNet-18 起始通道数 64
        self.ret_feat = ret_feat
        ...
        # 分支结构，每个分支有独立的 layer4 和分类器
        self.branches = nn.ModuleList()
        self.classifiers = nn.ModuleList()
        shared_inplanes = self.in_planes  # 保存共享部分的通道数（即 layer3 的输出通道数）
        for _ in range(num_branches):
            branch_layer4 = self._make_layer(block, nf * 8, num_blocks[3], stride=2)  # 每个分支独立的 layer4 (512通道)
            self.in_planes = shared_inplanes  # 每次重置，确保每个分支输入都是共享特征的通道数
            classifier = nn.Linear(dim_in, num_classes, bias=bias)
            self.branches.append(branch_layer4)
            self.classifiers.append(classifier)

        # # 控制分支 (用于计算分支权重)
        # self.control_fc = nn.Linear(nf * 4, num_branches)
        # self.control_bn = nn.BatchNorm1d(num_branches)
        #
        # # 归一化池化
    ...
    def logits(self, x):
        ...
        for i in range(self.num_branches):
            branch_out = self.branches[i](out)  # 每个分支独立的 layer4
            branch_out = avg_pool2d(branch_out, 4)
            branch_out = branch_out.view(branch_out.size(0), -1)
            branch_out = self.classifiers[i](branch_out)
            # 使用detach()分离分支输出，确保不参与反向传播
            # if i > 0:  # 假设你只希望第一个分支参与反向传播
            #     branch_out = branch_out.detach()  # Detach所有后续分支的输出
            outputs.append(branch_out)

        # 计算最终的加权输出
        # x_m = sum(x_c[:, i].unsqueeze(1) * outputs[i] for i in range(self.num_branches))
        # x_m = sum(outputs) / self.num_branches
    ...
    def forward(self, x):
        ...
        # x_c = F.relu(self.control_bn(self.control_fc(x_c)))
        # x_c = F.softmax(x_c, dim=1)

        outputs = []

        for i in range(self.num_branches):
            branch_out = self.branches[i](x)  # 每个分支独立的 layer4
            branch_out = self.avgpool(branch_out).view(branch_out.size(0), -1)
            if i == 0:
                out_feature = branch_out
            branch_out = self.classifiers[i](branch_out)
            outputs.append(branch_out)
        ...
        # return outputs + [x_m]
        return out_feature
def MultiBranchResNet18(nclasses, dim_in=512, nf=64, bias=True, ret_feat=True):
    return MultiBranchResNet(BasicBlock, [2, 2, 2, 2], nclasses, nf, bias, dim_in=dim_in, ret_feat=ret_feat)


class WTBasicBlock(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, use_wt=True):
        super(WTBasicBlock, self).__init__()
        # 第一个卷积层 - 负责通道数变化和下采样，使用标准卷积
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes) if bn else nn.Identity()

        if use_wt:
            from src.models.WTConv2d import WTConv2d
            self.conv2 = WTConv2d(in_channels=planes, out_channels=planes, kernel_size=3)
        else:
...
class WTResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf, bias, dim_in=512,
                 instance_norm=False, input_channels=3, ret_feat=True, use_wt=True):
        super(WTResNet, self).__init__()
        self.ret_feat = ret_feat
        self.in_planes = nf
        self.use_wt = use_wt
        ...
        if use_wt:
            from src.models.WTConv2d import WTConv2d
            self.conv1 = WTConv2d(in_channels=input_channels, out_channels=input_channels, kernel_size=3)
            # 额外的1x1卷积来变换通道数
            self.channel_conv = nn.Conv2d(input_channels, nf, kernel_size=1, bias=False)
        else:
            self.conv1 = nn.Conv2d(input_channels, nf, kernel_size=3, stride=1, padding=1, bias=False)
            self.channel_conv = nn.Identity()

        self.bn1 = nn.BatchNorm2d(nf) if bn else nn.Identity()
        self.instance_norm = instance_norm
        if self.instance_norm:
        ...
        self.mu = OutlookAttention(dim=64, num_heads=8)
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            if block == WTBasicBlock:
                layers.append(block(self.in_planes, planes, stride, self.use_wt))
            else:
                layers.append(block(self.in_planes, planes, stride))
...
def WTResNet18(nclasses, dim_in=512, nf=64, bias=True, ret_feat=True, use_wt=True):
    return WTResNet(WTBasicBlock, [2, 2, 2, 2], nclasses, nf, bias,
                    dim_in=dim_in, ret_feat=ret_feat, use_wt=use_wt)

...
class WaveletTransformCPU(Function):
    @staticmethod
    def forward(ctx, input, filters):
        ctx.save_for_backward(filters)
        # 确保数据类型匹配
        filters = filters.to(dtype=input.dtype, device=input.device)
    ...
    def backward(ctx, grad_output):
        ...
        # 确保数据类型匹配
        filters = filters.to(dtype=grad_output.dtype, device=grad_output.device)
        
        grad = inverse_wavelet_transform(grad_output, filters)
        return grad, None


class InverseWaveletTransformCPU(Function):
    @staticmethod
    def forward(ctx, input, filters):
        ctx.save_for_backward(filters)
        # 确保数据类型匹配
...
class WTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        ...
        # 不再使用预初始化的函数，而是直接使用类
        self.wt_function = WaveletTransformCPU.apply
        self.iwt_function = InverseWaveletTransformCPU.apply

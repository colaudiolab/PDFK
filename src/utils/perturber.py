...
class Perturber():
        ...
        with torch.no_grad():
            for w in model.parameters():
                if w.dim() <= 1:
                    ...
                    w.add_(torch.randn_like(w) * torch.norm(
                        w) * EPS)  # z *torch.norm(w) * self.gamma) # This is changed for ablation

    def perturb_model(self, X, y):
        out_o = F.softmax(self.net(X)).detach()
        self.proxy.load_state_dict(self.net.state_dict())  # 复制当前模型 → proxy，加入小随机噪声（仅用于扰动）
        ...
        for idx in range(self.steps):  # to have multiple steps (set to 1 step by default)
            pertopt.zero_grad()
            loss = -(F.kl_div(F.log_softmax(self.proxy(X), dim=1), out_o, reduction='none').sum(
    ...
    def get_loss(self, X, y):
        ...
        out_n = self.net(X)
        if outs is not None:
            loss_kl = self.lam * (F.kl_div(F.log_softmax(out_n, dim=1), outs, reduction='none').sum(
                dim=1) * mask).sum() / mask.sum()
            return loss_kl
        else:
            return None
